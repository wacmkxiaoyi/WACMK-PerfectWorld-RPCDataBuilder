    ...
    def clean_str(ss):
        def stack(type_str):
            def to_jsonstr(input_str):
                input_str = input_str.replace("&lt", "[").replace("&gt", "]").replace("<","[").replace(">","]").replace(";","")
            ...
            def res_stack(stacks):
                nstacks=[stacks[0]]
                for i in stacks[1:]:
                    if type(i) in (list,tuple): nstacks[-1]=[nstacks[-1],res_stack(i)[0] if nstacks[-1] in ("std::vector",) else res_stack(i)]
                    else: nstacks.append(i)
                return nstacks
        ...
        return stack(ss)
    def rep_str(ss): 
        if type(ss) in (list,tuple):  return [rep_str(i) for i in ss]
        elif "GNET::" == ss[:len("GNET::")]: return f"{ss[len('GNET::'):]}"
        elif ss in maps: return maps[ss]
        return ss
...
def output(clsname,isclass="True",filename="result.py",file_call="call.dat",file_name="source.dat"):
    ...
    for i in range(len(call)): strc+=("\t" if isclass.lower()!="false" else "")+f"\t\"{name[i]}\":{call[i]},\n"
    if strc[-2]==",": strc=strc[:-2]+"\n"+("\t" if isclass.lower()!="false" else "")+"\t}\n"
    else: strc=strc[:-1]+"}\n"
    with open(filename,"w") as file: file.writelines(strc)
